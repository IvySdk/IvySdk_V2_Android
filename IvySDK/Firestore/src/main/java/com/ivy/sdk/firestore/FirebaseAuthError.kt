...
//
//
//enum class FirebaseAuthError(val description: String) {
//    ERROR_INVALID_AUTH_CODE("The server auth code is null. Please check the documentation."),
...
//    ERROR_PROVIDER_ALREADY_LINKED("User has already been linked to the given provider"),
//    ERROR_NETWORK("A request to a service has failed due to a network error. Inspect the device's network connectivity state or retry later to resolve."),
//    ERROR_UNKNOWN("An unknown error occurred.");
//
//    companion object {
//        fun fromException(paramException: Exception?): FirebaseAuthError {
//            try {
//                if (paramException is FirebaseAuthException) return FirebaseAuthError.valueOf(
//                    paramException.errorCode
//                )
//                if (paramException is FirebaseNetworkException) return ERROR_NETWORK
//                if (paramException is FirebaseTooManyRequestsException) return ERROR_TOO_MANY_REQUESTS
//                if (paramException is FirebaseApiNotAvailableException) return ERROR_API_NOT_AVAILABLE
//                ILog.e(
//                    "FirebaseAuthError",
//                    "exception:${paramException?.javaClass?.name};${paramException} "
//                )
