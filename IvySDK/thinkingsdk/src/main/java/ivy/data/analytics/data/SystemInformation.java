...
package ivy.data.analytics.data;
...
public class SystemInformation {
    ...
    private Map<String, Object> setupDeviceInfo(Context mContext) {
        ...
        if (!TDPresetProperties.disableList.contains(TDConstants.KEY_COUNTRY)) {
            deviceInfo.put(TDConstants.KEY_COUNTRY, Locale.getDefault().getCountry());
        }
        if (!TDPresetProperties.disableList.contains(TDConstants.KEY_CURRENCY)) {
            try {
                Currency currency = Currency.getInstance(Locale.getDefault());
                deviceInfo.put(TDConstants.KEY_CURRENCY, currency.getCurrencyCode());
            } catch (Exception e) {
            }
        }
        ...
        if (mTimeZone != null
            ...
            //to-do
            deviceInfo.put(TDConstants.KEY_INSTALL_TIME, installTime.getUTCTime());
            deviceInfo.put(TDConstants.KEY_INSTALL_TIME_IT, firstInstallTime);
        }
        if (mTimeZone != null
                && !TDPresetProperties.disableList.contains(TDConstants.KEY_UPDATED_TIME)) {
            //TDTime installTime = new TDTime(new Date(packageInfo.firstInstallTime), mTimeZone);
            TDTime installTime = new TDTime(new Date(lastUpdatedTime), mTimeZone);
            //to-do
            deviceInfo.put(TDConstants.KEY_UPDATED_TIME, installTime.getUTCTime());
            deviceInfo.put(TDConstants.KEY_UPDATED_TIME_UT, lastUpdatedTime);
        }
...
//        }
        ...
        if (!TDPresetProperties.disableList.contains(TDConstants.KEY_SIMULATOR)) {
            deviceInfo.put(TDConstants.KEY_SIMULATOR, EmulatorDetector.isEmulator() ? 1 : 0);
        }
//        return deviceInfo;
