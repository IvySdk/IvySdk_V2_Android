        ...
        if (properties != null && TDLog.mEnableLog) {
            for (Iterator iterator = properties.keys(); iterator.hasNext(); ) {
                ...
                if (TextUtils.isEmpty(key)) {
                    TDLog.w(TAG, "Empty property name is not allowed.");
                    //return false;
                }

                if (!(KEY_PATTERN.matcher(key).matches()) && !DEFAULT_KEYS.contains(key)) {
                    TDLog.w(TAG, "[ThinkingData] Warning: Incorrect properties name[" + key + "]. "
                ...
                try {
                    ...
                    if (!(value instanceof String
                        ...
                        TDLog.w(TAG, "[ThinkingData] Warning: Property value must be type String, "
                    ...
                    if (value instanceof Number) {
                        ...
                        if (number > 9999999999999.999 || number < -9999999999999.999) {
                            TDLog.w(TAG, "[ThinkingData] Warning: The number value [" + value + "] is invalid.");
                            //return false;
                        }
                    }
                } catch (JSONException e) {
                    TDLog.e(TAG, "Unexpected parameters." + e);
                    return false;
                }
            }
        }
